...
void gsum1(uint32_t total_mem, uint32_t elastic_mem, uint32_t univmon_mem, double count_ratio, uint32_t d,
	...
	for (int i = 0; i < T; i++) {
		...
		double eu_sum = eu.gsum(g);
		double hu_sum = hu.gsum(g);
		double su_sum = su.gsum(g);
		double lu_sum = lu.gsum(g);
		double univmon_sum = univmon.gsum(g);
		double sota_sum = 0;
		if (*g != sqr)
		{
			sota_sum = sota.gsum(g);
		}
		else
		{
			sota_sum = sota.innerProduction(sota);
		}
...
void config(char* argv[])
{
	...
	output_file = to_string(dataset_type) + "_estimation_" + echo + ".csv";
