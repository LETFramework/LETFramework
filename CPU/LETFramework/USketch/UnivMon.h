...
public:
	...
	void batch_insert(zmq::socket_t& socket)
	{
		...
		while (true)
		{
			...
			key_type key = swap_pair.first;
			int f = swap_pair.second;
			element_num += f;
			sketches[0]->insert((uint8_t*)&key, f);
		}
	}
	void batch_insert1(zmq::socket_t& socket)
	{
		zmq::message_t message;
		key_type key;
		int f;
		while (true)
		{
			...
			for (int i = 1; i < level; i++)
			{
				int hash_val = hash[i]->run((const char*)&key, key_len) & 1;
				if (hash_val)
				{
					sketches[i]->insert((uint8_t*)&key, f);
				}
				else
				{
					break;
