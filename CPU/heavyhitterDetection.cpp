...
void gsum1_subset(uint32_t total_mem, uint32_t elastic_mem, uint32_t univmon_mem, double count_ratio, uint32_t d,
	...
	SOTA sota(total_mem, 0, 1.0 / 4, 1.0 / 4, 1.0 / 4, 1.0 / 4);
	for (int i = 0; i < T; i++) {
		eu.initial();
		hu.initial();
		su.initial();
...
void gsum_subset(uint32_t total_mem, double topk_ratio, double count_ratio, uint32_t d, double (*g)(double), double lambda, ofstream& out) {
...
void config(char* argv[])
{
	...
	output_file = to_string(dataset_type) + "_heavyHitterDetection_" + echo + ".csv";
...
int main(int argc, char* argv[]) {
	...
	{
		gsum_subset(total_mem, topk_ratio, count_ratio, d, g, lambda, out);
	}
	out.close();
	return 0;
}