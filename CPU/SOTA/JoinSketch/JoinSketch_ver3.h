...
public:
	...
	long double Join(Short_C_Sketch*other,double(*g)(double)= [](double x)->double {return x ; }){
		long double res[MAX_HASH_NUM];
		for (int i = 0; i < d; i++){
			long double k=0;
			for (int j = 0; j < w; j++)
				k+=1ll*g(double(counter[i][j])* double(other->counter[i][j]));
			res[i]=k;
		}
		sort(res, res + d);
		if (d % 2 == 0)
	...
	long double gsum(double (*g)(double))
	{
		long double res[MAX_HASH_NUM];
		for (int i = 0; i < d; i++) {
			long double k = 0;
...
public:
	...
	long double Join(Sketch* _other,double (*g)(double)) {
		Classifier* other = (Classifier*)_other;
		long double re = Light->Join(other->Light,g);
		for (int i = 0; i < Heavy->n; i++)
			for (int j = 0; j < Heavy->m; j++)
				if (Heavy->solid(i, j)) {
					re += 1.0 * g(double(Heavy->Key[i][j]) * double(other->Query(Heavy->Id[i][j])));
				}
		for (int i = 0; i < other->Heavy->n; i++)
			for (int j = 0; j < other->Heavy->m; j++)
				if (other->Heavy->solid(i, j)) {
					re += 1.0 * g(double(QueryMid(other->Heavy->Id[i][j])) * double(other->Heavy->Key[i][j]));
				}
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++)
				if (Mid[i][j].Key != 0) {
					re += 1.0 * g(double(Mid[i][j].Key) * double(other->QueryMid(Mid[i][j].Id)));
				}
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++)
				if (other->Mid[i][j].Key != 0) {
					re += 1.0 * g(double(other->Mid[i][j].Key) * double(Light->Query(other->Mid[i][j].Id)));
				}
		return re;
	}
	...
	long double gsum(double(*g)(double))
	{
		long double re = Light->gsum(g);
		for (int i = 0; i < Heavy->n; i++)
			for (int j = 0; j < Heavy->m; j++)
				if (Heavy->solid(i, j)) {
					re += 1.0 * g(Heavy->Key[i][j]);
				}
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++)
				if (Mid[i][j].Key != 0) {
					re += 1.0 * g(Mid[i][j].Key);
				}
		return re;
	}
	~Classifier()
