...
public:
	...
	struct Bucket {
		...
		count_type Query(const data_type item, uint32_t seed_s,
			...
			uint32_t choice = hash32(item, seed_s) & 1;
			uint32_t whichcast = hash32(item, seed_incast) % counter_num;
			count_type retv = (only_heavy == true) ? 0 : max(incast[whichcast] * COUNT[choice], 0);
			for (uint32_t i = 0; i < slot_num; ++i) {
				if (items[i] == item) {
	...
	void get_heavy_hitters(int threshold, vector<pair<string, uint32_t>>& results)
	{
		for (uint32_t i = 0; i < BUCKET_NUM; i++)
		{
			for (uint32_t j = 0; j < slot_num; j++)
			{
				if (abs(buckets[i].counters[j]) >= threshold)
				{
					results.push_back(make_pair(string((const char*)&buckets[i].items[j], 4), buckets[i].counters[j]));
				}
			}
		}
	}
	double gsum(double (*g)(double))
	{
		...
		for (uint32_t i = 0; i < BUCKET_NUM; i++)
		{
			for (uint32_t j = 0; j < slot_num; j++)
			{
				double x = abs(buckets[i].counters[j]);
				ret += g(x);
			}
		}
		return ret;
