#ifndef _FrequentBASED_H
#define _FrequentBASED_H
class FrequentBased :public TopkPart
{
#define JUDGE_IF_SWAP(min_val, guard_val) ((guard_val) > ((min_val)))
public:
    uint32_t* nCounters;
    FrequentBased(uint32_t _mem_in_bytes) :TopkPart(_mem_in_bytes) {
    ...
    uint32_t query(key_type key) {
        ...
        for (int i = 0; i < MAX_VALID_COUNTER; ++i)
            if (buckets[pos].key[i] == key)
                return GetCounterVal(buckets[pos].val[i]);
        return 0;
    }
